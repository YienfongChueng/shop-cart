        ...
        <div class="item-list-wrap">
          <div class="cart-item">
            ...
            <ul class="cart-item-list">
              <li v-for="item of cartList" :key="item.productId">
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <a href="javascipt:;" @click="editCart('checked',item)" :class="['checkbox-btn', 'item-check-btn', {'checked': item.checked}]">
        ...
        <div class="cart-foot-wrap">
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a href="javascipt:;" @click="toggleCheckAll">
                  <span :class="['checkbox-btn', 'item-check-btn', {'check':checkAllFlag}]">
                    <svg class="icon icon-ok">
            ...
            <div class="cart-foot-r">
              ...
              <div class="btn-wrap">
                <a :class="['btn btn--red', {'btn--dis': !checkedCount}]" @click="checkOut">结算</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    checkAllFlag () {
      // 当数组中所有对象都返回true，整体才会返回true，否则，返回false
    ...
    totalPrice () {
      ...
      this.cartList.forEach((item, index) => {
        if (item.checked) {
          totalPrice += item.productPrice * item.productNum
        }
      })
      return totalPrice
    }
  ...
  methods: {
    init () {
      Api.CartApi.getCartData().then(res => {
        if (res.data.length > 0) {
          this.cartList = res.data
        }
      })
    },
    editCart (type, values) {
      if (type === 'sub') {
        if (values.productNum > 0) {
          values.productNum--
        }
      } else if (type === 'add') {
        values.productNum++
